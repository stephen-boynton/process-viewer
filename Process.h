...
public:
  Process(string _pid, string _user, string _cmd, float _cpu, int _uptime) : PID(_pid), user(_user), CMD(_cmd), CPU(_cpu), upTime(_uptime){};
  Process(string _pid) : PID(_pid){};
  ...
  void SetPID(string _pid) { PID = _pid; };
